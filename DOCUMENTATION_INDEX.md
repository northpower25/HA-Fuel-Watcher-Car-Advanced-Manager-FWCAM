# haFWCMA - Documentation Overview

This file provides an overview of all available documentation for the Fuel Watcher Car Advanced Manager (haFWCMA) integration.

---

## 📚 Quick Navigation

### 🇩🇪 German Documentation
- **[DOKUMENTATION_DE.md](DOKUMENTATION_DE.md)** - Vollständige deutsche Dokumentation
- **[BLUEPRINTS_DE.md](BLUEPRINTS_DE.md)** - Blueprint-Dokumentation und Anleitungen
- **[HACS_INSTALLATION_DE.md](HACS_INSTALLATION_DE.md)** - HACS-Installationsanleitung

### 🇬🇧 English Documentation
- **[README.md](README.md)** - Main documentation and quick start
- **[HACS_INSTALLATION.md](HACS_INSTALLATION.md)** - HACS installation guide

---

## 📖 Documentation by Topic

### Getting Started
1. **Installation**
   - [HACS Installation (EN)](HACS_INSTALLATION.md)
   - [HACS Installation (DE)](HACS_INSTALLATION_DE.md)

2. **Configuration**
   - [Vehicle Entity Integration](docs/VEHICLE_ENTITIES.md)
   - [Telegram Setup (EN)](docs/TELEGRAM_SETUP.md)
   - [Telegram Setup (DE)](docs/TELEGRAM_SETUP_DE.md)

### Features & Usage

3. **Core Features**
   - [Entity Documentation](docs/ENTITIES.md)
   - [Refueling Log Guide](docs/REFUELING_LOG_GUIDE.md)
   - [Refueling Log Guide (DE)](docs/REFUELING_LOG_GUIDE_DE.md)
   - [Trip Tracking](docs/TRIP_TRACKING_README.md)
   - [Data Update Frequencies](docs/DATA_UPDATE_FREQUENCIES.md)
   - [Data Update Frequencies (DE)](docs/DATA_UPDATE_FREQUENCIES_DE.md)

4. **Advanced Features**
   - [Geolocation Concept (DE)](docs/GEOLOCATION_CONCEPT.md)
   - [Geolocation Concept (EN)](docs/GEOLOCATION_CONCEPT_EN.md)
   - [API Documentation](docs/API.md)
   - [Data Storage](docs/DATA_STORAGE.md)

### Automation & Blueprints

5. **Blueprints**
   - **[BLUEPRINTS_DE.md](BLUEPRINTS_DE.md)** - Complete blueprint documentation (German)
   - [blueprints/automation/](blueprints/automation/) - Automation blueprints
   - [blueprints/script/](blueprints/script/) - Script blueprints
   - [blueprints/README.md](blueprints/README.md) - Blueprint overview

6. **Available Blueprints**
   - 🔔 Low Fuel Alert
   - 💰 Price Drop Notification
   - ⏰ Smart Refueling Reminder
   - 🚗 Automatic Trip Logging
   - 📍 Geolocation Proximity (planned)
   - ✍️ Manual Refuel Entry Script
   - 🛣️ Trip Completion Script
   - 🔍 Fuel Price Query Script

### Troubleshooting & Support

7. **Troubleshooting**
   - [Troubleshooting Guide](docs/TROUBLESHOOTING.md)
   - [Telegram Troubleshooting (DE)](docs/TELEGRAM_TROUBLESHOOTING_DE.md)
   - [Telegram Troubleshooting (EN)](docs/TELEGRAM_TROUBLESHOOTING_EN.md)
   - [FWCAM Card Troubleshooting](docs/FWCAM_CARD_TROUBLESHOOTING.md)

8. **Technical Documentation**
   - [Developer Notes](DEVELOPER_NOTES.md)
   - [Contributing Guide](docs/CONTRIBUTING.md)
   - [Security Audit Summary (DE)](SECURITY_AUDIT_SUMMARY_DE.md)
   - [Data Quality Indicators](docs/DATA_QUALITY_INDICATORS.md)

---

## 🎯 Documentation by User Type

### For New Users
**Start here:**
1. [README.md](README.md) - Overview and features
2. [HACS_INSTALLATION.md](HACS_INSTALLATION.md) - Installation
3. [docs/VEHICLE_ENTITIES.md](docs/VEHICLE_ENTITIES.md) - Connect your vehicle
4. [docs/TELEGRAM_SETUP.md](docs/TELEGRAM_SETUP.md) - Optional: Telegram setup

**German users:**
1. [DOKUMENTATION_DE.md](DOKUMENTATION_DE.md) - Komplett-Anleitung

### For Automation Enthusiasts
**Blueprint resources:**
1. [BLUEPRINTS_DE.md](BLUEPRINTS_DE.md) - Blueprint documentation
2. [blueprints/](blueprints/) - Browse available blueprints
3. [docs/TELEGRAM_REFUELING_BOT.md](docs/TELEGRAM_REFUELING_BOT.md) - Advanced Telegram integration

### For Developers
**Technical documentation:**
1. [DEVELOPER_NOTES.md](DEVELOPER_NOTES.md) - Development guidelines
2. [docs/dev_docs/ENTITY_ATTRIBUTES_GUIDELINES.md](docs/dev_docs/ENTITY_ATTRIBUTES_GUIDELINES.md) - Entity attribute structure standards
3. [docs/dev_docs/ATTRIBUTE_STRUCTURE_IMPLEMENTATION_SUMMARY.md](docs/dev_docs/ATTRIBUTE_STRUCTURE_IMPLEMENTATION_SUMMARY.md) - Attribute standardization summary
4. [docs/API.md](docs/API.md) - API documentation
5. [docs/DATA_STORAGE.md](docs/DATA_STORAGE.md) - Storage schema
6. [docs/TRIP_TRACKING_CONCEPT.md](docs/TRIP_TRACKING_CONCEPT.md) - Trip tracking architecture

### For Troubleshooters
**Problem-solving resources:**
1. [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md) - General issues
2. [docs/TELEGRAM_TROUBLESHOOTING_DE.md](docs/TELEGRAM_TROUBLESHOOTING_DE.md) - Telegram issues
3. [TODO.md](TODO.md) - Known issues and roadmap

---

## 📑 Complete Documentation Index

### Root Directory
- [README.md](README.md) - Main documentation
- [DOKUMENTATION_DE.md](DOKUMENTATION_DE.md) - German comprehensive guide
- [BLUEPRINTS_DE.md](BLUEPRINTS_DE.md) - Blueprint documentation (German)
- [TODO.md](TODO.md) - Roadmap and feature status
- [DEVELOPER_NOTES.md](DEVELOPER_NOTES.md) - Developer guidelines
- [CHANGE-HISTORY.md](CHANGE-HISTORY.md) - Version history

### Installation
- [HACS_INSTALLATION.md](HACS_INSTALLATION.md)
- [HACS_INSTALLATION_DE.md](HACS_INSTALLATION_DE.md)

### Configuration & Setup
- [docs/VEHICLE_ENTITIES.md](docs/VEHICLE_ENTITIES.md)
- [docs/TELEGRAM_SETUP.md](docs/TELEGRAM_SETUP.md)
- [docs/TELEGRAM_SETUP_DE.md](docs/TELEGRAM_SETUP_DE.md)

### Features
- [docs/ENTITIES.md](docs/ENTITIES.md)
- [docs/REFUELING_LOG_GUIDE.md](docs/REFUELING_LOG_GUIDE.md)
- [docs/REFUELING_LOG_GUIDE_DE.md](docs/REFUELING_LOG_GUIDE_DE.md)
- [docs/TRIP_TRACKING_README.md](docs/TRIP_TRACKING_README.md)
- [docs/TRIP_TRACKING_CONCEPT.md](docs/TRIP_TRACKING_CONCEPT.md)
- [docs/DATA_UPDATE_FREQUENCIES.md](docs/DATA_UPDATE_FREQUENCIES.md)
- [docs/DATA_UPDATE_FREQUENCIES_DE.md](docs/DATA_UPDATE_FREQUENCIES_DE.md)

### Advanced Features
- [docs/GEOLOCATION_CONCEPT.md](docs/GEOLOCATION_CONCEPT.md)
- [docs/GEOLOCATION_CONCEPT_EN.md](docs/GEOLOCATION_CONCEPT_EN.md)
- [docs/GEOLOCATION_SUMMARY.md](docs/GEOLOCATION_SUMMARY.md)
- [docs/TELEGRAM_REFUELING_BOT.md](docs/TELEGRAM_REFUELING_BOT.md)
- [docs/TELEGRAM_REFUELING_BOT_DE.md](docs/TELEGRAM_REFUELING_BOT_DE.md)

### Technical
- [docs/API.md](docs/API.md)
- [docs/DATA_STORAGE.md](docs/DATA_STORAGE.md)
- [docs/DATA_QUALITY_INDICATORS.md](docs/DATA_QUALITY_INDICATORS.md)
- [docs/CONTRIBUTING.md](docs/CONTRIBUTING.md)

### Developer Documentation
- [docs/dev_docs/ENTITY_ATTRIBUTES_GUIDELINES.md](docs/dev_docs/ENTITY_ATTRIBUTES_GUIDELINES.md)
- [docs/dev_docs/ATTRIBUTE_STRUCTURE_IMPLEMENTATION_SUMMARY.md](docs/dev_docs/ATTRIBUTE_STRUCTURE_IMPLEMENTATION_SUMMARY.md)
- [docs/dev_docs/DEVELOPER_NOTES.md](docs/dev_docs/DEVELOPER_NOTES.md)

### Troubleshooting
- [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md)
- [docs/TELEGRAM_TROUBLESHOOTING_DE.md](docs/TELEGRAM_TROUBLESHOOTING_DE.md)
- [docs/TELEGRAM_TROUBLESHOOTING_EN.md](docs/TELEGRAM_TROUBLESHOOTING_EN.md)
- [docs/FWCAM_CARD_TROUBLESHOOTING.md](docs/FWCAM_CARD_TROUBLESHOOTING.md)

### UI Components
- [docs/FWCAM_CARD_VISUAL_GUIDE.md](docs/FWCAM_CARD_VISUAL_GUIDE.md)

### Blueprints
- [blueprints/README.md](blueprints/README.md)
- [blueprints/automation/](blueprints/automation/)
- [blueprints/script/](blueprints/script/)

### Implementation Summaries
- [IMPLEMENTATION_SUMMARY.md](IMPLEMENTATION_SUMMARY.md)
- [IMPLEMENTATION_SUMMARY_CUSTOM_CARD.md](IMPLEMENTATION_SUMMARY_CUSTOM_CARD.md)
- [IMPLEMENTATION_SUMMARY_HISTORICAL_DATA.md](IMPLEMENTATION_SUMMARY_HISTORICAL_DATA.md)
- [IMPLEMENTATION_SUMMARY_ISSUE_128.md](IMPLEMENTATION_SUMMARY_ISSUE_128.md)

### Security & Audit
- [SECURITY_AUDIT_SUMMARY.md](SECURITY_AUDIT_SUMMARY.md)
- [SECURITY_AUDIT_SUMMARY_DE.md](SECURITY_AUDIT_SUMMARY_DE.md)
- [PII_REMEDIATION_GUIDE.md](PII_REMEDIATION_GUIDE.md)

---

## 🔍 Finding Specific Information

### "How do I..."

**...install the integration?**
→ [HACS_INSTALLATION.md](HACS_INSTALLATION.md) or [HACS_INSTALLATION_DE.md](HACS_INSTALLATION_DE.md)

**...connect my car?**
→ [docs/VEHICLE_ENTITIES.md](docs/VEHICLE_ENTITIES.md)

**...set up Telegram?**
→ [docs/TELEGRAM_SETUP.md](docs/TELEGRAM_SETUP.md) or [docs/TELEGRAM_SETUP_DE.md](docs/TELEGRAM_SETUP_DE.md)

**...create automations?**
→ [BLUEPRINTS_DE.md](BLUEPRINTS_DE.md) and [blueprints/](blueprints/)

**...use the trip tracking feature?**
→ [docs/TRIP_TRACKING_README.md](docs/TRIP_TRACKING_README.md)

**...log refueling via Telegram?**
→ [docs/TELEGRAM_REFUELING_BOT_DE.md](docs/TELEGRAM_REFUELING_BOT_DE.md)

**...troubleshoot issues?**
→ [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md)

**...understand what each entity does?**
→ [docs/ENTITIES.md](docs/ENTITIES.md)

### "What is..."

**...the complete feature list?**
→ [DOKUMENTATION_DE.md](DOKUMENTATION_DE.md) Section 6

**...the roadmap?**
→ [TODO.md](TODO.md)

**...the data structure?**
→ [docs/DATA_STORAGE.md](docs/DATA_STORAGE.md)

**...the API format?**
→ [docs/API.md](docs/API.md)

---

## 🌐 Language Availability

| Document Type | 🇩🇪 German | 🇬🇧 English |
|---------------|-----------|----------|
| Main Documentation | ✅ DOKUMENTATION_DE.md | ✅ README.md |
| Installation Guide | ✅ HACS_INSTALLATION_DE.md | ✅ HACS_INSTALLATION.md |
| Blueprint Guide | ✅ BLUEPRINTS_DE.md | ⏳ Planned |
| Telegram Setup | ✅ TELEGRAM_SETUP_DE.md | ✅ TELEGRAM_SETUP.md |
| Refueling Guide | ✅ REFUELING_LOG_GUIDE_DE.md | ✅ REFUELING_LOG_GUIDE.md |
| Geolocation | ✅ GEOLOCATION_CONCEPT.md | ✅ GEOLOCATION_CONCEPT_EN.md |
| Troubleshooting | ✅ TELEGRAM_TROUBLESHOOTING_DE.md | ✅ TELEGRAM_TROUBLESHOOTING_EN.md |

---

## 📝 Documentation Standards

All documentation follows these principles:
- **Clarity**: Simple, understandable language
- **Structure**: Logical progression from simple to complex
- **Examples**: Practical code examples and use cases
- **Accuracy**: Verified against implementation
- **Completeness**: All features documented

---

## 🤝 Contributing to Documentation

Found an error or want to improve documentation?

1. Check [DEVELOPER_NOTES.md](DEVELOPER_NOTES.md) for guidelines
2. Review [docs/CONTRIBUTING.md](docs/CONTRIBUTING.md)
3. Create an issue or pull request

---

**Last Updated**: 2024-02-17  
**Documentation Version**: 1.0.0  
**Integration Version**: See [manifest.json](custom_components/hafwcma/manifest.json)
